...
window.onload = function () {
    ...
    var lorentzian = new Renderer(document.getElementById('lorentzian'), LORENTZIAN_WIDTH, LORENTZIAN_HEIGHT, function (m, b, k, omegaf) {
        var amplitudeVsOmega = function (omega) {
            return 1 / sqrt(sq(k / m - sq(omega)) + sq(omega * b / m));
        };
    ...
    var m, k, b, f0, omegaf;
    control('m', id, id, function (m_) {
        m = m_;
        output();
    });
    control('b', exp0, log, function (b_) {
        b = b_;
        output();
    });
    control('k', exp, log, function (k_) {
        k = k_;
        output();
    });
    var ff = control('f', id, id, function (f_) {
        f0 = f_;
    });
    var omegaff = control('omegaf', id, id, function (omegaf_) {
        omegaf = omegaf_;
        output();
    });
