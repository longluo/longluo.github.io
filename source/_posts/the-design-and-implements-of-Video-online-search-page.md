...
- [一、在线搜索](#一在线搜索)
  - [1.1 在线搜索实现效果](#11-在线搜索实现效果)
    - [1.1.1 热词界面](#111-热词界面)
    - [1.1.2 关联词](#112-关联词)
    - [1.1.3 搜索结果分类浏览](#113-搜索结果分类浏览)
    - [1.1.4 语音搜索](#114-语音搜索)
    - [1.1.5 语音搜索结果](#115-语音搜索结果)
- [二、在线搜索UI设计及实现](#二在线搜索ui设计及实现)
  - [2.1 SearchBar](#21-searchbar)
  - [2.2 热词和关联词](#22-热词和关联词)
  - [2.3 分类筛选页面](#23-分类筛选页面)
    - [2.3.1 水平ListView和ListView实现方案](#231-水平listview和listview实现方案)
      - [2.3.1.1 iQiyi 搜索结果](#2311-iqiyi-搜索结果)
      - [2.3.1.2 iQiyi实现](#2312-iqiyi实现)
    - [2.3.2 滑动实现方案](#232-滑动实现方案)
- [三、在线搜索代码逻辑](#三在线搜索代码逻辑)
  - [3.1 搜索类](#31-搜索类)
  - [3.2 搜索结果频道分类](#32-搜索结果频道分类)
  - [3.3 搜索结果页面](#33-搜索结果页面)
- [四、问题及解决](#四问题及解决)
  - [4.1 无法输入中文问题](#41-无法输入中文问题)
  - [4.2 视频异常退出问题](#42-视频异常退出问题)
    - [4.2.1 滑动页面异常退出](#421-滑动页面异常退出)
    - [4.2.2 选择列表一个热词同时按下返回键，视频退出](#422-选择列表一个热词同时按下返回键视频退出)
...
![热词显示](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_hotwords.png?imageView/1/w/540/h/960/q/62/format/PNG)

### 1.1.2 关联词
当**搜索文本输入框含有文字**时，会获取当前输入文字，以此为关键词获取网络的一些联想词，可以点击此联想词发起一次搜索。

![关联词显示](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_asswords.png?imageView/1/w/540/h/960/q/62/format/PNG)
...
![搜索结果](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_results.png?imageView/1/w/540/h/960/q/62/format/PNG)

分频道浏览：

![搜索结果分类](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_scrollfilter.png?imageView/1/w/540/h/960/q/62/format/PNG)
...
![语音搜索](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_voicesearch.png?imageView/1/w/540/h/960/q/62/format/PNG)

### 1.1.5 语音搜索结果

![语音搜索结果](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_voiceresults.png?imageView/1/w/540/h/960/q/62/format/PNG)
...
![iQiyi搜索结果](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/iqiyi_search_filter.png?imageView/1/w/540/h/960/q/62/format/PNG)
...
![搜索类](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_classes.png)
...
![Weights](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_weights.png)
...
![initFragment](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_initFragment.png)

在不同搜索结果页面进行切换时，实现一个监听器，获取当前选中页面：

![pageChangeListener](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_pagechangelistener.png)

在具体页面实现中，我们需要获取相关的参数及绘制页面：

![resultFragment](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_resultFragment.png)
...
![TextWatcher](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_textwatcher.png)
...
![搜索框setText](http://qrn8zhq7m.bkt.clouddn.com/images/2014/Android/VideoSearch/search_setText.png)
