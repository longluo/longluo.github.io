...
keywords: Android, App, JSON解析, XML解析, XML, JSON, Parser,  
---
...
- [一、XML](#一xml)
  - [1.1 XML属性](#11-xml属性)
    - [1.1.1 XML与HTML的主要差异](#111-xml与html的主要差异)
    - [1.1.2 XML是不作为的。](#112-xml是不作为的)
    - [1.1.3 XML仅仅是纯文本](#113-xml仅仅是纯文本)
    - [1.1.4 XML允许自定义标签](#114-xml允许自定义标签)
    - [1.1.5 XML不是对HTML的替代](#115-xml不是对html的替代)
  - [1.2 XML的语法](#12-xml的语法)
      - [1.2.1 所有元素都必须有关闭标签](#121-所有元素都必须有关闭标签)
      - [1.2.2 XML标签对大小写敏感](#122-xml标签对大小写敏感)
      - [1.2.3 XML标签对大小写敏感](#123-xml标签对大小写敏感)
      - [1.2.4 XML文档必须有根元素](#124-xml文档必须有根元素)
      - [1.2.5 XML的属性值须加引号](#125-xml的属性值须加引号)
      - [1.2.6 实体引用](#126-实体引用)
      - [1.2.7 XML中的注释](#127-xml中的注释)
      - [1.2.8 以 LF 存储换行](#128-以-lf-存储换行)
    - [1.3 XML CDATA](#13-xml-cdata)
      - [1.3.1 PCDATA](#131-pcdata)
      - [1.3.2 转义字符](#132-转义字符)
      - [1.3.3 CDATA](#133-cdata)
  - [1.4 举个栗子](#14-举个栗子)
        - [iQiyi提供的电影频道的XML电影数据如下：](#iqiyi提供的电影频道的xml电影数据如下)
          - [这是展示一部电影的具体数据，包括标题、介绍、内容、导演、演员、时长、上映年份等很多内容。](#这是展示一部电影的具体数据包括标题介绍内容导演演员时长上映年份等很多内容)
  - [1.5 XML树结构](#15-xml树结构)
    - [1.5.1 一个XML文档实例](#151-一个xml文档实例)
  - [1.6 XML DOM](#16-xml-dom)
    - [1.6.1 定义](#161-定义)
        - [DOM将XML文档作为一个树形结构，而树叶被定义为节点。](#dom将xml文档作为一个树形结构而树叶被定义为节点)
    - [1.6.2 总结](#162-总结)
  - [1.7 XML如何解析？](#17-xml如何解析)
    - [1.7.1 Step 1](#171-step-1)
        - [XML文件是一棵树，首先需要找到对应的节点，然后从节点开始解析，比如搜索找到的就是result/weights/weight 和result/weights/weight 2个节点，分别从这个开始解析：](#xml文件是一棵树首先需要找到对应的节点然后从节点开始解析比如搜索找到的就是resultweightsweight-和resultweightsweight-2个节点分别从这个开始解析)
    - [1.7.2 Step 2](#172-step-2)
    - [1.7.3 Step 3](#173-step-3)
    - [1.7.4 Step 4](#174-step-4)
        - [以上就是搜索数据的XML的解析和数据展示过程。](#以上就是搜索数据的xml的解析和数据展示过程)
- [二、JSON](#二json)
  - [2.1 JSON是什么？](#21-json是什么)
  - [## 2.2 JSON格式](#-22-json格式)
      - [***JSON***构建于两种结构：](#json构建于两种结构)
      - [***JSON***具有以下这些形式：](#json具有以下这些形式)
  - [2.3 举个栗子](#23-举个栗子)
        - [iQiyi提供的电影频道的JSON电影数据如下：](#iqiyi提供的电影频道的json电影数据如下)
          - [从上面的例子可以很清晰的看出JSON是如何展示一个电影的数据的，当然这是JSON格式化之后的数据。JSON的元数据是不便于阅读的。](#从上面的例子可以很清晰的看出json是如何展示一个电影的数据的当然这是json格式化之后的数据json的元数据是不便于阅读的)
  - [2.4 如何解析JSON？](#24-如何解析json)
    - [<1>. 常见方法](#1-常见方法)
    - [<2>. Android中如何创建JSON？](#2-android中如何创建json)
    - [<3>. 如何解析JSON？](#3-如何解析json)
          - [第一步，需要从网络服务器上发起请求，获取到JSON数据：](#第一步需要从网络服务器上发起请求获取到json数据)
          - [第二步，获取到对应的对应的JSONObject数据：](#第二步获取到对应的对应的jsonobject数据)
          - [获取到JSON Object之后，就对这个JSONObject进行解析：](#获取到json-object之后就对这个jsonobject进行解析)
        - [以上展示的就是JSON数据的解析过程。](#以上展示的就是json数据的解析过程)
    - [<4>. Android JSON解析库](#4-android-json解析库)
  - [# 三、 JSON vs. XML](#-三-json-vs-xml)
    - [<1>. JSON相比XML的不同之处](#1-json相比xml的不同之处)
      - [总之： JSON 比 XML 更小、更快，更易解析。](#总之-json-比-xml-更小更快更易解析)
    - [<2>. XML和JSON的区别：](#2-xml和json的区别)
