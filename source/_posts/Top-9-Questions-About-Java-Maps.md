...
In Java, Map interface provides three collection views: key set, value set, and keyvalue
set. All of them can be converted to List by using a constructor or addAll()
method. The following snippet of code shows how to construct an ArrayList from
a map.
/ / k ey l i s t
Li s t keyLi s t = new Ar rayLi s t (map. keySet ( ) ) ;
/ / v a l u e l i s t
Li s t va lueLi s t = new Ar rayLi s t (map. valueSet ( ) ) ;
/ / key􀀀v a l ue l i s t
Li s t ent r yLi s t = new Ar rayLi s t (map. ent rySe t ( ) ) ;
...
returns a key-value set, therefore the most efficient way of going through every
entry of a map is
for ( Entry ent ry : map. ent rySe t ( ) ) {
/ / g e t k ey
K key = ent ry . getKey ( ) ;
/ / g e t v a l u e
...
I t e r a t o r i t r = map. ent rySe t ( ) . i t e r a t o r ( ) ;
while ( i t r . hasNext ( ) ) {
...
null key􀀀value | yes􀀀yes | yes􀀀yes | no􀀀yes
synchronized | no | yes | no
time performance | O( 1 ) | O( 1 ) | O( log n)
implementation | buckets | buckets | red􀀀black t r e e
